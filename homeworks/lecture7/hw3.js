...
const server = http.createServer((req, res) => {
  ...
  if (method === "GET") {
    if (url === "/") {
      ...
      fs.readFile(path.join(__dirname, "home.html"), "utf8", (err, html) => {
        if (err) {
          ...
          let title = parsedUrl.query.title ? parsedUrl.query.title : "";
          let content = parsedUrl.query.content? parsedUrl.query.content: "";
          const modifiedHtml = html.replace('{{title}}', title).replace('{{content}}', content);
    ...
    if (url === "/create-post") {
      ...
      req.on("end", () => {
        const parsedBody = Buffer.concat(body).toString();  //string: title=A&content=V
        let queryString = "home.html?" + parsedBody;
        res.writeHead(302, {
